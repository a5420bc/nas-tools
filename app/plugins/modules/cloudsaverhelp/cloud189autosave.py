...
class Cloud189AutoSaveSDK(object):
    ...
    def create_task(self, account_id: str, share_link: str, target_folder_id: str = None, 
                   target_folder: str = None, overwrite_folder: bool = False) -> TaskResponse:
        """创建任务"""
        if not self.enabled:
            log.error('Cloud189AutoSave 未配置或未启用')
    ...
    def save_to_cloud(self, account_id: str, share_link: str, target_folder: str = None, 
                     overwrite: bool = False) -> bool:
        """保存分享链接到云盘（完整流程）"""
        log.info(f'开始保存分享链接到云盘: {share_link}')
